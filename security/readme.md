...
6. **`needs`** — конструкция, которая задаёт зависимость между Jobs. Например, `gate` с `needs: security` начнёт выполняться только после завершения `security`.
...
Pipeline - /AI-SEC-PLATFORM/.git/workflows/security.yml
...
# 3. Этап 3: Статический анализ кода (SAST)
    * Инструмент: Semgrep (универсальный, быстрый, мега-популярный) или SonarQube.
...
Перейдите в терминале в папку с вашим проектом (где находится локальный Git-репозиторий) и запустите:
```
